...
<template>
    ...
    <Head title="Search" />
    <AppLayout>
        <h1 class="mb-6 text-2xl font-semibold text-foreground">Search Posts</h1>
        ...
        <form @submit.prevent="search" class="mb-8 flex gap-2">
            <input v-model="searchQuery" type="text" placeholder="Search by title or content..."
                class="flex-1 rounded-md border border-border bg-background px-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
            <button type="submit"
