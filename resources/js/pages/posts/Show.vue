    ...
    <AppLayout>
        <article class="mx-auto w-full max-w-3xl px-4 sm:px-6">
            ...
            <div class="mb-8">
                ...
                <div v-if="post.data.tags?.length" class="flex flex-wrap gap-1.5">
                    <TagPill v-for="tag in post.data.tags" :key="tag.id" :tag="tag" />
                </div>
            </div>
            ...
            <div class="flex gap-6">
                <div class="sticky top-20 hidden shrink-0 sm:block">
                    <VoteButton votable-type="post" :votable-id="post.data.id" :initial-score="post.data.vote_score"
            ...
            <div class="mt-12 border-t border-border pt-8">
                <CommentsSection :post-slug="post.data.slug" :comments="post.data.comments ?? []" />
            </div>
        </article>
    </AppLayout>
</template>
